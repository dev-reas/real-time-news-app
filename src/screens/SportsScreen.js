...
import {
    ...
    responsiveScreenFontSize, responsiveScreenWidth
} from 'react-native-responsive-dimensions';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useTheme } from '@react-navigation/native';

export function SportsScreen(props) {
    ...
    const getPost = async () => {
        await axios.get(`${BASE_URL}/api/category/4`, {
            headers: {
    ...
    return (
        <>
            ...
            <ScrollView nestedScrollEnabled={true}
                ...
                <FlatlistHeader title={'Technology and Entertainment Section'} />
                {
                    categoriesPosts == ''
                        ?
                        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                            <View style={{
                                ...
                                <Text style={{ textAlign: 'justify', color: colors.text, fontSize: responsiveScreenFontSize(2) }}>No post yet in Technology and Entertainment.</Text>
